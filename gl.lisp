	  ...
	  (cond ((eq cmd 'r-cubic-bezier)
		 (let ((lines (expand-relative-bezier rest :n 13)))
		   (dolist (f lines)
		     (push f res))))
		((eq cmd 'cubic-bezier)
		 (let ((lines (expand-absolute-bezier rest :n 13)))
		   (dolist (f lines)
		     (push f res))))
		(t (push e res)))))
      (reverse res)))
  ...
  (defun svg-to-immediate-opengl (ls)
	...
	(destructuring-bind (cmd x y) e
	  (cond ((eq cmd 'line-to)
		 (push `(vertex ,x ,y) res))
		((eq cmd 'translate)
		 (push `(color 0 0 0) res)
		 (push `(vertex ,x ,y) res)
		 (push `(color 1 1 1) res))
		(t (break "unexpected command ~a" cmd)))))
      (reverse res)))
  )
...
(defmacro def-number-fun ()
  `(progn
     ,@(loop for i from 3 below 4 collect
	    (let ((one (first (cdr (assoc i *numerals*)))))
	      `(defun ,(intern (format nil "DRAW-~a-FUN" i)) ()
		 ,@(svg-to-immediate-opengl (accumulate-relative-coordinates 
					     (expand-all-relative-bezier-into-lines 
					      (svg-path-d-to-lisp (split-at-comma-space one))))))))))
#+nil
